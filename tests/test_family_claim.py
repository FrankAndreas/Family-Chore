...
def test_family_dashboard_claim_logic(db):
    ...
    user_p = crud.create_user(db, schemas.UserCreate(nickname=f"FD_Dad_{unique_id}", login_pin="0000", role_id=role_p.id))
    # Create User B (Teen)
    user_t = crud.create_user(db, schemas.UserCreate(nickname=f"FD_Son_{unique_id}", login_pin="0000", role_id=role_t.id))
    ...
    completed_instance = crud.complete_task_instance(db, instance_id=instance.id, actual_user_id=user_t.id)
    ...
    tx = db.query(models.Transaction).filter(models.Transaction.reference_instance_id == instance.id).first()
    ...
    db_task = crud.get_tasks(db, skip=0, limit=100)
    # The task definition should still be assigned to Role Parent or whatever it was
    original_task = db.query(models.Task).filter(models.Task.id == task.id).first()
