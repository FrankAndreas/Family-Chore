...
def redeem_split_even(client, seeded_db, reward_name, amount, context):
    ...
    reward = seeded_db.query(models.Reward).filter(models.Reward.name == reward_name).first()
...
@when(parsers.parse('they attempt to redeem "{reward_name}" with "{user1}" contributing {amount1:d} points and "{user2}" contributing {amount2:d} points'))
